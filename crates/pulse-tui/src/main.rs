...
fn purge_stale_subtitles() {
    tokio::task::spawn_blocking(|| {
        ...
        let mut dirs = vec![
            pulse_core::service::resolve_subtitle_dir(),
...
fn purge_stale_update_artifacts() {
    tokio::task::spawn_blocking(|| {
        if let Ok(current_exe) = std::env::current_exe() {
            pulse_core::updater::apply::cleanup_stale_update_artifacts(&current_exe);
        }
    });
}
...
async fn main() -> std::io::Result<()> {
    ...
    if let Some(pos) = args.iter().position(|a| a == "--proxy-for-vlc") {
        ...
        pulse_core::proxy::run_sidecar(target_url, headers, sub_url).await;
        return Ok(());
    }
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        println!("pulse-tui {}", env!("CARGO_PKG_VERSION"));
        println!("A terminal client for finding and streaming movies, TV shows, and anime.\n");
        println!("USAGE:");
        println!("    pulse-tui [OPTIONS]\n");
        println!("OPTIONS:");
    ...
    pulse_core::cache::clean_old_cache_background();

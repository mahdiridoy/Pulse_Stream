...
impl App {
    pub(super) fn resolve_download_base_dir(&self) -> std::path::PathBuf {
        pulse_core::service::resolve_download_dir(self.state.download_dir.as_deref())
    }
    ...
    pub(super) fn start_resilient_download(
        ...
        let raw_title = self
            ...
            .unwrap_or(pulse_core::download::DEFAULT_STREAM_NAME);
        let clean_title = pulse_core::providers::moviebox::clean_moviebox_title(raw_title);
        let is_series = self
            .state
            .selected_details
            .as_ref()
        ...
        let safe_title = pulse_core::download::safe_file_stem(&clean_title);
        ...
        let mut client_builder = pulse_core::net::http_client_builder()
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Some(subtitle_url) = subtitle_url {
                ...
                let lang_code = sub_lang
                    .as_deref()
                    .and_then(pulse_core::providers::moviebox::title::language_to_code)
            ...
            if is_dash {
                ...
                let result = pulse_core::download::download(
                    &client,
                    &link,
                    &destination,
                    cancel,
                ...
                match result {
                    Ok(pulse_core::download::DownloadOutcome::Completed { .. }) => {
                        sender
                            .send(Action::DownloadCompleted(
                                destination.to_string_lossy().into_owned(),
                            ))
                    ...
                    Ok(pulse_core::download::DownloadOutcome::Paused { .. }) => {
                        sender
                            .send(Action::DownloadPaused(
                                destination.to_string_lossy().into_owned(),
                            ))
...
pub(crate) fn yt_dlp_missing_guidance() -> String {
    if pulse_core::updater::artifact::is_termux_environment() {
...
mod tests {
    ...
    use pulse_core::providers::models::{ProviderKind, Release, SourceMirror};

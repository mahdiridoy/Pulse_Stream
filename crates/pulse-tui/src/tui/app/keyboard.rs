...
impl App {
    pub(super) async fn handle_key(&mut self, key: KeyEvent) -> Option<()> {
                ...
                if let KeyCode::Char('s') = key.code {
                    if !self.state.streaming_enabled {
                        ...
                        && self.state.active_provider
                            != pulse_core::providers::models::ProviderKind::Addons
                    {
        ...
        if self.state.input_mode != InputMode::Editing {
            if let Some((version, _)) = &self.state.update_available {
                let is_homebrew = std::env::current_exe()
                    .map(|p| pulse_core::updater::apply::is_homebrew_managed(&p))
                    .unwrap_or(false);
                ...
                match key.code {
                    ...
                    KeyCode::Char('o') | KeyCode::Char('O') => {
                        let url = pulse_core::updater::check::release_tag_url(version);
                        let _ = open::that(&url);
                        self.state.update_available = None;
                        return None;
                    }
        ...
        if self.state.show_browse_popup {
            let is_addon =
                self.state.active_provider == pulse_core::providers::models::ProviderKind::Addons;
            let total_count = if is_addon {
                pulse_core::providers::addons::models::curated_catalog_presets(
            ...
            match key.code {
                ...
                KeyCode::Up | KeyCode::Char('k') => {
                    crate::tui::state::cycle_list_selection(
                        ...
                        pulse_core::providers::models::ProviderKind::ENABLED.len(),
                        false,
                    );
                }
                KeyCode::Down | KeyCode::Char('j') => {
                ...
                KeyCode::Enter | KeyCode::Char(' ') => {
                    ...
                    if let Some(&provider) =
                        pulse_core::providers::models::ProviderKind::ENABLED.get(idx)
                    {
                        self.action_sender
                            ...
                            if self.state.favorites_focus {
                                ...
                                if idx < self.state.search_results.len() {
                                    ...
                                    self.state.history.recent.retain(|i| {
                                        !(i.subject_id == res.id
                                            && (pulse_core::providers::models::ProviderKind::parse(
                                                &i.provider,
                                            ) == Some(res.provider)
...
mod tests {
    ...
    async fn test_normal_mode_jump_navigation() {
        ...
        app.state.search_results = vec![
            pulse_core::models::SearchResult {
                id: "1".to_string(),
                title: "Movie 1".to_string(),
                stype: 1,
                release_year: "2020".to_string(),
                provider: pulse_core::models::ProviderKind::MovieBox,
                cover_url: None,
                season: 0,
                episode: 0,
            },
    ...
    async fn test_delete_history_item_with_key() {
        ...
        let item = pulse_core::history::WatchHistoryItem {
    ...
    async fn test_normal_mode_c_clears_search_and_results() {
        ...
        app.state.search_results = vec![pulse_core::models::SearchResult {
            id: "1".to_string(),
    ...
    async fn test_browse_esc_returns_to_homepage() {
        ...
        app.state.active_screen = crate::tui::state::Screen::Home;
        app.state.input_mode = InputMode::Normal;
        app.state.active_browse_preset = Some(pulse_core::models::BrowsePreset::Trending);
        app.state.search_results = vec![pulse_core::models::SearchResult {
            id: "1".to_string(),
    ...
    async fn test_provider_popup_keyboard_navigation() {
        ...
        app.state.active_screen = crate::tui::state::Screen::Home;
        app.state.input_mode = InputMode::Normal;
        app.state.active_provider = pulse_core::models::ProviderKind::MovieBox;
    ...
    async fn test_details_overview_modal_keyboard_toggle_and_scroll() {
        ...
        app.state.selected_details = Some(pulse_core::providers::models::MediaDetails {
            ...
            media_type: pulse_core::providers::models::MediaType::Movie,

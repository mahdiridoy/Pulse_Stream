...
impl App {
    ...
    fn current_favorite_candidate(&self) -> Option<pulse_core::favorites::FavoriteItem> {
        match self.state.active_screen {
            Screen::Details => {
                let details = self.state.selected_details.as_ref()?;
                let provider = self.current_subject_provider().cache_key().to_string();
                Some(pulse_core::favorites::FavoriteItem::from_details(
                    &provider, details,
                ))
            }
            Screen::Home => {
                if self.state.favorites_focus {
                    ...
                    match self.state.effective_home_deck_tab() {
                        crate::tui::state::HomeDeckTab::ContinueWatching => {
                            ...
                            Some(pulse_core::favorites::FavoriteItem {
    ...
    pub(super) fn open_favorite(&mut self, index: usize) {
        ...
        self.state.selected_details = Some(pulse_core::models::MediaDetails::from_search_result(

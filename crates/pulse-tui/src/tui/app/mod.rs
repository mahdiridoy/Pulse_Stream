...
use pulse_core::providers::models::RequestContext;
...
pub struct App {
    state: AppState,
    theme: Theme,
    service: std::sync::Arc<pulse_core::service::MovieBoxService>,
...
impl App {
    pub fn new() -> Self {
        ...
        let provider_was_sanitized = !state.provider_enabled(config.active_provider)
            && config.active_provider != pulse_core::providers::models::ProviderKind::Addons;
        ...
        if !state.addons_enabled
            && state.active_provider == pulse_core::providers::models::ProviderKind::Addons
        {
            state.active_provider = pulse_core::providers::models::ProviderKind::MovieBox;
        }
        ...
        state.installed_addons = pulse_core::config::load_addons();
        ...
        let service = std::sync::Arc::new(pulse_core::service::MovieBoxService::new());
    ...
    fn save_installed_addons(&self) {
        pulse_core::config::save_addons(&self.state.installed_addons);
    }

    fn load_installed_addons_from_config(&mut self) {
        self.state.installed_addons = pulse_core::config::load_addons();
    }

    fn save_tv_playlists(&self) {
        let Some(path) = pulse_core::config::tv_path() else {
            return;
        };
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if let Err(error) = pulse_core::cache::atomic_write_file(&path, json.as_bytes()) {
                ...
                Ok(content) => match serde_json::from_str::<Vec<String>>(&content) {
                    ...
                    Err(e) => {
                        ...
                        log::error!(
                            ...
                            pulse_core::logging::sanitize_path(&path),
                            pulse_core::logging::sanitize_path(&corrupt_path)
                        );
    ...
    fn reload_tv_playlists(&self) {
        ...
        tokio::spawn(async move {
            let parser = pulse_core::providers::tv::M3UParser::new();

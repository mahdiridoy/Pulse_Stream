...
impl App {
        ...
        std::io::Error: From<<B as ratatui::backend::Backend>::Error>,
    {
        ...
        self.state.available_players = crate::player::detect();
    ...
    fn draw_sources_picker(&mut self, frame: &mut Frame, area: Rect) {
        if self.state.show_sources_popup {
            let providers = pulse_core::providers::models::ProviderKind::ENABLED;
    ...
    fn draw_update_modal(&self, frame: &mut Frame, area: Rect) {
        ...
        if let Some((version, notes)) = &self.state.update_available {
            ...
            let env = std::env::current_exe()
                .ok()
                .map(|p| pulse_core::updater::apply::detect_environment(&p))
                .unwrap_or(pulse_core::updater::apply::InstallationEnvironment::DirectReplace);
            ...
            match env {
                pulse_core::updater::apply::InstallationEnvironment::Homebrew => {
                    text.push(
                        Line::from(vec![
                ...
                pulse_core::updater::apply::InstallationEnvironment::Termux => {
                    text.push(
                ...
                pulse_core::updater::apply::InstallationEnvironment::Flatpak => {
                    text.push(
                        Line::from(vec![Span::styled(
                ...
                pulse_core::updater::apply::InstallationEnvironment::Snap => {
                    text.push(
                        Line::from(vec![Span::styled(
                ...
                pulse_core::updater::apply::InstallationEnvironment::ReadOnly => {
                    text.push(
                        Line::from(vec![Span::styled(
                ...
                pulse_core::updater::apply::InstallationEnvironment::DirectReplace
                | pulse_core::updater::apply::InstallationEnvironment::WindowsHelper => {}
            }

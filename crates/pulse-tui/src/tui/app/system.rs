...
impl App {
        ...
        match action {
                ...
                Screen::Details => {
                    if let Some(id) = self.state.active_subject_id.clone() {
                        ...
                        tokio::task::spawn_blocking(move || {
                            pulse_core::cache::invalidate_provider_stream_cache(
                                provider, &id_clone, se, ep,
                            );
                            pulse_core::cache::invalidate_provider_details_cache(
                                provider,
                                &id_clone_2,
                            );
                        });
            ...
            Action::ClearCache => {
                ...
                tokio::spawn(async move {
                    let result = tokio::task::spawn_blocking(pulse_core::cache::clear_all_cache)
                        .await
            ...
            Action::ToggleSettingsPopup => {
                ...
                if open {
                    self.reset_transient_overlays();
                    for player in crate::player::detect() {
            ...
            Action::SettingsResetDownloadDir => {
                ...
                self.persist_config();
                let default_dir =
                    pulse_core::logging::sanitize_path(self.resolve_download_base_dir());
                self.state.notify(
                    NotificationKind::Success,
                    "Download Folder",
                    ...
                    match self.state.settings_selected_row {
                        ...
                        1 => {
                            if self.state.available_players.is_empty() {
                                let detected = crate::player::detect();
                        ...
                        2 => {
                            if let Some(input) = self.state.settings_download_dir_input.take() {
                                    ...
                                    crate::tui::state::AppState::expand_download_path(new_path)
                                {
                                    ...
                                    self.state.download_dir = None;
                                    self.persist_config();
                                    let default_dir = pulse_core::logging::sanitize_path(
            ...
            Action::ShowBrowseMenu => {
                ...
                if current_mode == crate::tui::state::AppMode::Tv {
                    ...
                    && self.state.active_provider
                        != pulse_core::providers::models::ProviderKind::MovieBox
                {
                    self.state
            ...
            Action::CheckForUpdates => {
                ...
                tokio::spawn(async move {
                    let check_future =
                        pulse_core::updater::check_release(env!("CARGO_PKG_VERSION"));
            ...
            Action::StartSelfUpdate => {
                ...
                tokio::spawn(async move {
                    let release = match cached_release {
                        Some(r) => r,
                        None => {
                            match pulse_core::updater::check_release(env!("CARGO_PKG_VERSION"))
                                .await
                            {
                                Ok(Some(r)) => r,
                                Ok(None) => {
                    ...
                    let outcome =
                        pulse_core::updater::perform_self_update(&release, Some(&progress_tx))
                            .await;
            ...
            Action::SelfUpdateComplete(result) => {
                ...
                match result {
                    Ok(pulse_core::updater::SelfUpdateOutcome::Success) => {
                        self.state
                        ...
                        if let Ok(exe_path) = std::env::current_exe() {
                            if let Err(e) = pulse_core::updater::restart_process(&exe_path) {
                                log::error!("failed to restart process after update: {e}");
                            }
                        }
                        std::process::exit(0);
                    }
                    Ok(pulse_core::updater::SelfUpdateOutcome::RequiresManualUpgrade(msg)) => {
            ...
            Action::CheckBdixNetwork => {
                ...
                tokio::spawn(async move {
                    ...
                    let (circleftp, dhakaflix) = tokio::join!(
                        pulse_core::net::probe_url(
                            pulse_core::providers::bdix::circleftp::client::POSTS_URL,
                            timeout,
                        ),
                        async {
                            for (server, _) in
                                pulse_core::providers::bdix::dhakaflix::client::SERVERS
                            {
                                if pulse_core::net::probe_url(server, timeout).await {
                                    return true;
                                }
                            }
                            false
            ...
            Action::BdixProbeResult {
                ...
                if !self.state.provider_enabled(self.state.active_provider) {
                    let next = self
                        ...
                        .into_iter()
                        .next()
                        .unwrap_or(pulse_core::providers::models::ProviderKind::MovieBox);
    ...
    fn toggle_provider(&mut self, provider: pulse_core::providers::models::ProviderKind) {
        let currently_enabled = self.state.provider_enabled(provider);
        if currently_enabled {
            let would_remain: Vec<pulse_core::providers::models::ProviderKind> =
                pulse_core::providers::models::ProviderKind::ENABLED
                    .into_iter()

...
use pulse_core::providers::models::ProviderKind;
...
impl App {
    pub(super) fn resolve_playback_player(
        &self,
        source: &pulse_core::providers::models::PlaybackSource,
    ) -> PlaybackResolution {
        ...
        if let Some(chosen) = preferred {
            if self.state.available_players.contains(&chosen) {
                if crate::player::supports_headers(chosen, &source.headers) {
                ...
                let compatible_alternatives = self
                    ...
                    .iter()
                    .copied()
                    .filter(|&k| k != chosen && crate::player::supports_headers(k, &source.headers))
                    .collect::<Vec<_>>();
        ...
        if let Some(player) = self
            ...
            .iter()
            .copied()
            .find(|kind| crate::player::supports_headers(*kind, &source.headers))
        {
    ...
    pub(super) fn dispatch_playback_or_notify(
        ...
        match self.resolve_playback_player(&source) {
            ...
            PlaybackResolution::NoPlayersInstalled => {
                self.state.is_resolving_playback = false;
                self.state.pending_playback_source = None;
                let message = if pulse_core::updater::artifact::is_termux_environment() {
    ...
    fn build_watch_history_item(&self) -> Option<pulse_core::history::WatchHistoryItem> {
        ...
        if let Some(details) = &self.state.selected_details {
            let mut item = pulse_core::history::WatchHistoryItem::from_details(
        ...
        Some(pulse_core::history::WatchHistoryItem {
    ...
    pub(super) fn launch_player(
        ...
        if let (Some((p, s, se, ep)), Some(item)) = (&tracker_opts, &history_item) {
            if let Some(state_path) = crate::player::tracker::state_file_path(p, s, *se, *ep) {
                let initial_state = pulse_core::history::PendingPlaybackState::from_item(
                    item,
                    ...
                    tokio::task::spawn_blocking(move || {
                        if let Err(e) = std::fs::write(&state_path, serialized) {
                            log::warn!(
                                ...
                                pulse_core::logging::sanitize_path(&state_path)
                            );
                        }
                    });
                }
        ...
        tokio::spawn(async move {
            ...
            {
                let download_res = pulse_core::service::MovieBoxService::new()
                ...
                match download_res {
                    ...
                    Err(_) => {
                        ...
                        log::warn!(
                            ...
                            kind,
                            pulse_core::logging::sanitize_url(url)
                        );
            ...
            let (effective_link, effective_subtitle) = if needs_proxy {
                match pulse_core::proxy::spawn_sidecar(&link, &headers, subtitle.as_deref()) {
                    Ok(local_url) => {
                        let sub_url =
            ...
            let mut command = crate::player::command(
                kind,
                &effective_link,
            ...
            match command.spawn() {
                ...
                Err(error) => {
                    log::error!(
                        ...
                        kind,
                        pulse_core::logging::sanitize_url(&link)
                    );
                    if let Some(path) = temporary_subtitle {
    ...
    pub(super) async fn handle_playback(&mut self, action: Action) -> Option<()> {
        match action {
            Action::PlayStream => {
                    ...
                    || self.current_subject_provider().is_bdix()
                {
                    if let Some(release) = self.get_selected_release() {
                        ...
                        let direct_source = pulse_core::providers::models::PlaybackSource {
                    ...
                    if let Some(rid) = resource_id {
                        ...
                        tokio::spawn(async move {
                            let cached = tokio::task::spawn_blocking({
                                ...
                                let rid = rid.clone();
                                move || {
                                    pulse_core::cache::get_captions_cache_typed(&subject_id, &rid)
                                }
                            })
                            .await
                            .ok()
                            ...
                            match result {
                                Ok(Ok(res)) => {
                                    ...
                                    tokio::task::spawn_blocking(move || {
                                        pulse_core::cache::set_captions_cache_typed(
                                            &subject_id,
                                            &rid,
                                            &res_for_cache,
                                        );
            ...
            Action::ShowSubtitlePopup(link, subtitles) => {
                ...
                if options.len() > 1 {
                    ...
                    if let Some(source) = self.state.pending_playback_source.take() {
                        ...
                        let source = pulse_core::providers::models::PlaybackSource {
            ...
            Action::LaunchPlayback(kind, source) => {
                ...
                self.state.player_picker_popup = false;
                self.state.last_playback_launch = std::time::Instant::now();
                if !crate::player::supports_headers(kind, &source.headers) {
                    self.state.notify(
                        NotificationKind::Error,
            ...
            Action::PlayerCrashed(code, error_msg) => {
                ...
                let is_termux = pulse_core::updater::artifact::is_termux_environment();
...
mod tests {
    ...
    async fn show_popup_when_subtitles_available() {
        ...
        let ext_captions = vec![
            pulse_core::providers::models::SubtitleOption {
    ...
    async fn test_get_selected_resource_id_resolution() {
        ...
        app.state.selected_resources = vec![
            pulse_core::providers::models::Release {
                provider: pulse_core::providers::models::ProviderKind::MovieBox,
    ...
    async fn test_play_stream_notifies_preparing_playback_accurately() {
        let mut app = crate::tui::app::App::new();
        app.state.active_provider = pulse_core::providers::models::ProviderKind::MovieBox;
        app.state.active_screen = crate::tui::state::Screen::Details;
        app.state.selected_resources = vec![pulse_core::providers::models::Release {
            provider: pulse_core::providers::models::ProviderKind::MovieBox,
            filename: "Movie.1080p.mkv".to_string(),
            quality: Some("1080p".to_string()),
            codec: Some("hevc".to_string()),
            ...
            season: None,
            episode: None,
            mirrors: vec![pulse_core::providers::models::SourceMirror {
    ...
    async fn test_android_player_allows_unauthenticated_and_referer_streams() {
        ...
        let bdix_source = pulse_core::providers::models::PlaybackSource {
            provider: pulse_core::providers::models::ProviderKind::BdixCircleFtp,
        ...
        let fourk_source = pulse_core::providers::models::PlaybackSource {
            provider: pulse_core::providers::models::ProviderKind::FourKHdHub,
        ...
        let auth_source = pulse_core::providers::models::PlaybackSource {
            provider: pulse_core::providers::models::ProviderKind::MovieBox,
            url: "https://example.com/index.mpd".to_string(),
            headers: vec![("Cookie".to_string(), "CloudFront-Policy=test".to_string())],
            subtitle: None,
    ...
    async fn test_playback_captures_active_subject_and_episode() {
        ...
        app.state.selected_details = Some(pulse_core::providers::models::MediaDetails {
            id: pulse_core::providers::models::ProviderMediaId {
                provider: pulse_core::providers::models::ProviderKind::MovieBox,
                value: "root_subject_100".to_string(),
            },
            title: "Test Series".to_string(),
            media_type: pulse_core::models::MediaType::Series,
            year: None,
            description: None,
            tagline: None,
            imdb_rating: None,
        ...
        let mirror = pulse_core::providers::models::SourceMirror {

...
use pulse_core::providers::addons::models::InstalledAddon;
    ...
    pub(super) async fn handle_addons(&mut self, action: Action) -> Option<()> {
        match action {
            Action::SwitchToStreamingMode => {
                ...
                if self.state.active_provider == pulse_core::providers::models::ProviderKind::Addons
                {

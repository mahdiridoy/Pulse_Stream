...
use pulse_core::models::{Episode, MediaType, ProviderKind, Release, Season};
        ...
        match action {
            ...
            Action::SuggestSuccess(request_id, query, suggestions) => {
                ...
                for raw_title in suggestions.into_iter().take(limit) {
                    let clean_title = if query.starts_with('/') {
                        ...
                        pulse_core::providers::moviebox::clean_moviebox_title(&raw_title)
                    };
            ...
            Action::SearchSuccess {
                ...
                for item in items {
                    ...
                    let stype = if item.media_type == pulse_core::models::MediaType::Series {
                        2
                    } else {
                        1
                    };
            ...
            Action::FetchPreview(id) => {
                            ...
                            if !cover_url.is_empty() {
                                ...
                                tokio::spawn(async move {
                                    if let Ok(Some(bytes)) = tokio::task::spawn_blocking({
                                        ...
                                        move || {
                                            pulse_core::cache::get_namespaced_image_cache(
                                                "iptv", &id_clone,
                                            )
                                        }
                                    })
                                        ...
                                        network::fetch_poster_bytes(&client, &cover_url).await
                                    {
                                        ...
                                        let _ = tokio::task::spawn_blocking(move || {
                                            pulse_core::cache::set_namespaced_image_cache(
                                                "iptv",
                                                &id_clone,
                                                &bytes_clone,
                                            )
                ...
                tokio::spawn(async move {
                    if let Ok(Some(cached_disk)) = tokio::task::spawn_blocking({
                        ...
                        move || pulse_core::cache::get_provider_details_cache_typed(prov, &id_clone)
                    })
                    .await
                    {
                        sender
                    ...
                    match service.details_typed(prov, &id_clone).await {
                        Ok(details) => {
                            ...
                            let _ = tokio::task::spawn_blocking(move || {
                                pulse_core::cache::set_provider_details_cache_typed(
                    ...
                    && let Some(url) = details.cover_url()
                {
                    ...
                    tokio::spawn(async move {
                        if let Ok(Some(bytes)) = tokio::task::spawn_blocking({
                            let id_clone = id_clone.clone();
                            move || {
                                pulse_core::cache::get_namespaced_image_cache("posters", &id_clone)
                            }
                        })
                        .await
                        {
            ...
            Action::FetchEpisodeStreams {
                ...
                if !force_refresh {
                    ...
                    if let Ok(Some(cached)) = tokio::task::spawn_blocking(move || {
                        pulse_core::cache::get_provider_stream_cache_typed(
                ...
                if context.provider == ProviderKind::Addons {
                    ...
                    let addons = pulse_core::config::load_addons();
                    ...
                    self.request_tasks.streams = Some(tokio::spawn(async move {
                        ...
                        let (releases, blocked_addons) =
                            pulse_core::providers::addons::aggregate_streams(
                        ...
                        if !releases.is_empty() {
                            ...
                            tokio::task::spawn_blocking(move || {
                                pulse_core::cache::set_provider_stream_cache_typed(
                                    provider,
                                    &id_clone,
                                    season,
                                    episode,
                        ...
                        let result = match context.provider {
                            ProviderKind::FourKHdHub => {
                                if let Some(client) = fourk_client.as_ref() {
                                    pulse_core::providers::ReleaseProvider::episode_streams(
                                        client, &id, season, episode,
                                    )
                                    .await
                                } else {
                                    Err(pulse_core::providers::models::ProviderError::Unavailable(
                ...
                self.request_tasks.streams = Some(tokio::spawn(async move {
                    ...
                    if let Ok(streams) = pulse_core::providers::ReleaseProvider::episode_streams(
                        ...
                        loop {
                            ...
                            .await
                            {
                                Ok(Ok((items, pager))) => {
                                    ...
                                    for item in items {
                                        all_items.push(
                                            pulse_core::providers::moviebox::adapt::moviebox_resource_item_to_release(
                                                &item,
                                            ),
                                        );
                                    }

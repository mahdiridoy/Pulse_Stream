...
use pulse_core::models::CatalogItem;
use pulse_core::providers::models::{ProviderKind, RequestContext};
...
impl App {
    ...
    pub(super) fn prepare_addon_catalog_request(
        &mut self,
        target: &pulse_core::providers::addons::models::AddonCatalogTarget,
    ) -> RequestContext {
        self.state.active_search_request = self.state.active_search_request.wrapping_add(1);
        self.state.active_preview_request = self.state.active_preview_request.wrapping_add(1);
        self.state.is_homepage_mode = false;
        ...
        self.request_tasks.search = Some(tokio::spawn(async move {
            if !force_refresh {
                ...
                if let Ok(Some(cached)) = tokio::task::spawn_blocking(move || {
                    pulse_core::cache::get_provider_search_cache_typed(provider, &q, page)
                })
                .await
                {
                    sender
            ...
            match result {
                Ok(items) => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        pulse_core::cache::set_provider_search_cache_typed(
                            provider, &q, page, &cached,
                        );
                    });
                    sender
                        .send(Action::SearchSuccess {
    ...
    pub(super) fn run_details_request(
        ...
        self.request_tasks.details = Some(tokio::spawn(async move {
            if !force_refresh {
                ...
                if let Ok(Some(cached)) = tokio::task::spawn_blocking(move || {
                    pulse_core::cache::get_provider_details_cache_typed(
                        context.provider,
                        &id_for_cache,
                    )
                })
    ...
    pub(super) fn run_homepage_request(&mut self, tab_id: String, page: usize) {
        ...
        self.request_tasks.homepage = Some(tokio::spawn(async move {
            ...
            if let Ok(Some((items, metrics))) = tokio::task::spawn_blocking(move || {
                pulse_core::cache::get_homepage_cache_typed(&t_clone, page)
            })
            .await
            {
                sender
            ...
            match service.homepage(&tab_id, page).await {
                Ok((items, metrics)) => {
                    ...
                    let t_clone = tab_id.clone();
                    tokio::task::spawn_blocking(move || {
                        pulse_core::cache::set_homepage_cache_typed(
                            &t_clone,
                            page,
    ...
    pub(super) fn append_homepage_items(
        ...
        metrics_map: std::collections::HashMap<String, pulse_core::models::BrowseMetrics>,
    ) -> usize {
        let mut count = 0;
        for item in items {
            let id = item.id.value.clone();
            let clean_title = pulse_core::providers::moviebox::clean_moviebox_title(&item.title);
            ...
            for (id, cover_url, provider) in to_fetch {
                ...
                tokio::spawn(async move {
                    ...
                    if let Ok(Some(bytes)) = tokio::task::spawn_blocking({
                        ...
                        move || pulse_core::cache::get_namespaced_image_cache("posters", &id_c)
                    })
                    .await
                    {

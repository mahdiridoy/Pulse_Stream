    ...
    if let Some(&row_area) = row_rects.get(2) {
        ...
        let value_spans = if let Some(input) = &state.settings_download_dir_input {
            ...
            let path_str = state
                .download_dir
                .as_ref()
                .map(pulse_core::logging::sanitize_path)
                .unwrap_or_else(|| {
                    pulse_core::logging::sanitize_path(pulse_core::service::resolve_download_dir(
                        None,
                    ))
                });
...
fn render_content_modes_settings(frame: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    if let Some(&row_area) = row_rects.get(1) {
        ...
        let enabled_count = pulse_core::providers::models::ProviderKind::ENABLED
            .iter()

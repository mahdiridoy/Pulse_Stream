...
use pulse_core::models::MediaDetails;
use ratatui::{
    Frame,
...
pub fn draw(frame: &mut Frame, area: Rect, state: &mut AppState, theme: &Theme) {
    ...
    if !list.is_empty() {
            ...
            .map(|(i, file)| {
                ...
                let upload_by = if !clean_source.is_empty() && !is_redundant_source {
                    ...
                    match file.provider {
                        ...
                        pulse_core::providers::models::ProviderKind::FourKHdHub => {
                            "4KHDHub".to_string()
                        }
                        pulse_core::providers::models::ProviderKind::BdixCircleFtp => {
                            "CircleFTP".to_string()
                        }
                        pulse_core::providers::models::ProviderKind::BdixDhakaFlix => {
                            "DhakaFlix".to_string()
                        }
                        pulse_core::providers::models::ProviderKind::Addons => "Addon".to_string(),
                    }
                };
        ...
        let msg = if waiting_for_language {
            ...
            if state.active_provider == pulse_core::providers::ProviderKind::Addons {
                error.clone()
...
pub(crate) fn season_confirm_summary(state: &AppState) -> Vec<String> {
    ...
    let dest = state
        ...
        .unwrap_or_else(|| {
            pulse_core::service::resolve_download_dir(None)
                .to_string_lossy()
                .to_string()
        });
...
fn details_footer(
    ...
    let show_provider = state.active_provider != pulse_core::providers::ProviderKind::Addons
        && !state
            .selected_details
            .as_ref()
            .is_some_and(|d| d.id.provider == pulse_core::providers::ProviderKind::Addons);
...
mod tests {
    ...
    fn test_details_footer_contextual_hints() {
        ...
        state.active_provider = pulse_core::providers::ProviderKind::Addons;
        ...
        state.active_provider = pulse_core::providers::ProviderKind::MovieBox;

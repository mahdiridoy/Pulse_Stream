...
fn render_search_state(
    ...
    match view {
        ...
        SearchViewState::NoResults => {
            ...
            } else if state.active_provider == pulse_core::providers::models::ProviderKind::Addons {
                let (btn1_label, btn2_label) = if is_compact_btn {
                    ("[ Addons (", "[ Clear (")
                } else {
...
pub(crate) fn render_discover_landing(
    ...
    let presets = if state.active_provider == pulse_core::providers::models::ProviderKind::Addons {
        [
...
╚═╝     ╚═╝  ╚═════╝    ╚═══╝   ╚═╝ ╚══════╝ ╚═════╝   ╚═════╝  ╚═╝  ╚═╝"
    ...
    if state.show_browse_popup {
        let raw_labels: Vec<String> = if state.active_provider
            == pulse_core::providers::models::ProviderKind::Addons
        {
            pulse_core::providers::addons::models::curated_catalog_presets(&state.installed_addons)
                .into_iter()
...
mod tests {
    ...
    fn test_discover_landing_rendering_streaming_mode() {
        ...
        let mut state = AppState {
            ...
            streaming_enabled: true,
            active_provider: pulse_core::providers::models::ProviderKind::MovieBox,
            ..Default::default()
        };
    ...
    fn test_discover_landing_rendering_addon_mode() {
        ...
        let mut state = AppState {
            input_mode: InputMode::Normal,
            addons_enabled: true,
            active_provider: pulse_core::providers::models::ProviderKind::Addons,
            ..Default::default()
        };
        state.history.recent.clear();
        state.favorites.items.clear();
    ...
    fn test_landing_deck_continue_watching_rendering() {
        ...
        state
            .history
            .recent
            .push(pulse_core::history::WatchHistoryItem {
                provider: "moviebox".to_string(),
    ...
    fn test_search_result_selection_indicator_without_opaque_background() {
        ...
        let mut state = AppState {
            input_mode: InputMode::Normal,
            search_query: "Matrix".into(),
            search_results: vec![pulse_core::models::SearchResult {
                id: "test_1".into(),

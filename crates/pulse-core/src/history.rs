...
impl WatchHistoryItem {
    pub fn from_details(
        ...
        let duration_seconds = details
            .duration
            .as_deref()
            .and_then(crate::text_utils::parse_duration_seconds);
    ...
    pub fn formatted_progress(&self) -> String {
        let pos = crate::text_utils::format_duration(self.progress_seconds);
        if let Some(dur) = self.duration_seconds {
            if dur > 0 {
                return format!("{} / {}", pos, crate::text_utils::format_duration(dur));
            }
        }
        pos
    }

...
impl MovieBoxService {
    ...
    /// Create a service with a custom HTTP client for the addon client
    /// (used for SSRF-protected addon fetching in the web server).
    pub fn with_addon_http_client(addon_http: reqwest::Client) -> Self {
        let http_client = crate::net::http_client_builder()
            .timeout(std::time::Duration::from_secs(15))
            .build()
            .unwrap_or_default();
        ...
        Self {
            ...
            circleftp_client: CircleFtpClient::new(),
            dhakaflix_client: DhakaFlixClient::new(),
            addon_client: crate::providers::addons::AddonClient::with_client(addon_http),
            http_client,
        }
    }
    ...
    pub async fn episode_streams(
        ...
        match provider {
            ProviderKind::MovieBox => {
                ReleaseProvider::episode_streams(&self.client, id, season, episode).await
            }
            ProviderKind::FourKHdHub => {
                let fourk = self.fourk_client.as_ref().ok_or_else(|| {
                    ProviderError::Unavailable("4KHDHub is unavailable".to_string())
                })?;
                ReleaseProvider::episode_streams(fourk, id, season, episode).await
            }
            ProviderKind::BdixCircleFtp => {
                ReleaseProvider::episode_streams(&self.circleftp_client, id, season, episode).await
            }
            ProviderKind::BdixDhakaFlix => {
                ReleaseProvider::episode_streams(&self.dhakaflix_client, id, season, episode).await
            }
            ProviderKind::Addons => {
                let installed_addons = crate::config::load_addons();
                let is_series = true;
    ...
    pub async fn get_ext_captions(
        ...
        for sub in all_captions {
            let sanitized_lang = crate::text_utils::sanitize_language_label(&sub.name);
        ...
        deduplicated.sort_by(|a, b| {
            let clean_a = crate::text_utils::sanitize_language_label(&a.name);
            let clean_b = crate::text_utils::sanitize_language_label(&b.name);
